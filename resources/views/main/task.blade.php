  ...
  <a href="{{ route('task.create') }}" class="flex flex-row hover:opacity-80 items-center">
    <div class="text-white text-sm font-semibold bg-primer rounded-full px-4 py-2">
      Create Task
        ...
        <svg xmlns="http://www.w3.org/2000/svg" class="h-12 w-12 stroke-white fill-primer" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
<main class="bg-white flex gap-4 pb-14 px-6 pt-3 justify-between">
      ...
      @foreach ($tasks as $task)
        ...
        <a href="{{ route('task-detail', $task->id) }}" class="flex flex-col items-start rounded-lg bg-white shadow-md cursor-pointer">
          <div class="w-full flex flex-col items-start px-3 py-2">
            ...
            @if ($task->deadline !== NULL)
              <div class="flex flex-row justify-center items-center mt-1">
                <div class="flex flex-row items-center">
                  ...
                  <div class="ml-2 text-sm" cols="30" rows="5" >{{  date("F j, Y H:i",strtotime($task->deadline)) ?? '' }} </div>
                </div>
              </div>             
            @else
            @endif
           ...
           <!-- Icon -->
            @if ($task->description !== NULL || $task->link !== NULL || $task->photo !== NULL)
  ...
  <!-- IN PROGRESS -->
  <div class="section-task">
    <div class="flex items-center rounded-t-lg bg-orange-400 p-1"></div>
    <div class="flex justify-between items-center px-4 py-1">
  ...
  <div class="section-task">
    ...
    <!-- Garis Ijo -->
    <div class="flex items-center rounded-t-lg bg-green-400  p-1"></div>
    <div class="flex justify-between items-center px-4 py-1">
